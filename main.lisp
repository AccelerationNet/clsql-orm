...
(defvar *db-vendor* nil
  "The database type we are generating for as keyword (:mysql, :sqlite3, :odbc etc)" )
...
(defun clsql-column-definitions (table &key (db-vendor *db-vendor*)
                                            (schema *schema*)
...
translate its type, and declare an initarg"
  (let ((cols (list-columns db-vendor table schema)))
    (unless cols
          ...
          (with-accessors ((column column) (db-type db-type) (col-length col-length)
            ...
            (when (and
              ...
              (if (and fkey-table fkey-col)
                  ...
                  (warn "CLSQL-ORM: Could not generate a join for: ~a.~a.~a because we couldn't
read the relationship. Perhaps your permissions should be adjusted"
                        schema table column)))))))

(defun clsql-join-definition (home-table home-key foreign-table foreign-key
                              &key (generate-accessors t))
  "Creates the definition for the joins. Note that this does not handle multi-column foreign keys
at the moment. For that matter, if you wish to have custom names and the like, you'd best define
an inheriting class"
...
(defgeneric list-columns (db-vendor table &optional schema)
  (:documentation "Returns a list of
  ...
  (:method (db-vendor table &optional (schema *schema*))
...
(defgeneric list-tables (db-vendor &optional schema)
  (:method (db-vendor &optional (schema *schema*))
    (clsql:select [table_name] [table_type]
                  :from [information_schema.tables]
                  :flatp T
                  :where [= [UPPER [table_schema]] (string-upcase schema)])))

(defun column-diff (db-vendor table-1 table-2 &key (schema-1 *schema*) (schema-2 *schema*))
  (let ((cols1 (list-columns db-vendor table-1 schema-1))
        (cols2 (list-columns db-vendor table-2 schema-2)))
    (list
     (iter (for c in cols1)
...
(defun gen-view-class (table &key (db-vendor *db-vendor*)
                                  classname
                                  (is-view nil)
                                  (generate-joins t)
                                  (generate-accessors t)
...
naming conventions, it's best to define a class that inherits from your generated class."
  ...
  (ensure-strings (table schema)
    (let* ((*schema* schema)
           ...
           (columns (clsql-column-definitions db-vendor
                                              table
                                              :generate-accessors generate-accessors
...
(defun gen-view-classes (&key (db-vendor *db-vendor*)
                              (classes)
                              (generate-joins t)
                              (generate-accessors t)
                              (schema "public")
  ...
  (setf classes
        (if classes
            ...
            (list-tables db-vendor schema)))
  (iter (for (table type) in classes)
        (gen-view-class table
                        :db-vendor db-vendor
                        :generate-joins generate-joins
